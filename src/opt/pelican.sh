...
if [ -d "$PELICAN_DIR" ] && grep -qw pelican /tmp/prov
  ...
  if pyenv which pipenv >/dev/null 2>&1 && pyenv exec pipenv graph 2>/dev/null | grep -q ^pelican=
    ...
    [ -n "$PELICAN_CNF" ] || PELICAN_CNF=$(find "$PELICAN_DIR" -maxdepth 1 -name '*conf.py' | head -n1)
    [ -n "$PELICAN_SRV" ] || PELICAN_SRV=/srv/web$(pyenv exec pelican -s "$PELICAN_CNF" --print-settings SITEURL | tr -d "'\n" | cut -d/ -f4- | printf '/%s' "$(cat -)" | sed 's|/$||')
    rm -rf "$PELICAN_SRV"
